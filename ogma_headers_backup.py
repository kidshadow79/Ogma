...
def _header():
    ...
    with ui.element('div').style('display: flex; align-items: center; gap: 8px; position: fixed; top: 16px; right: 16px; z-index: 999;') as header_container:
        ...
        def toggle_cognitive_mirror():
            """Ouvre le popup de paramètres Introspection v2.1"""
            try:
                ...
                if _ensure_cognitive_mirror:
                    ...
                    if cognitive_mirror:
                        ...
                        # v2.1: IntrospectionEngine - afficher popup v2.1
                        if hasattr(cognitive_mirror, 'config') and hasattr(cognitive_mirror.config, 'is_enabled'):
                            is_active = cognitive_mirror.is_active if hasattr(cognitive_mirror, 'is_active') else False
                            print(f"[SUBCONSCIENCE] v2.1 - En cours: {is_active}")
                            
                            # Créer popup v2.1
                            _show_introspection_v21_popup(cognitive_mirror)
                            
                        # v1/v2.0: ui_components disponible
                    ...
                    with ui.expansion('📜 Instructions (avancé)', icon='code').classes('w-full').tooltip('Personnalisez les instructions données à chaque étape de l\'introspection'):
                        ...
                        with ui.tab_panels(instr_tabs, value=tab_s1).classes('w-full'):
                            with ui.tab_panel(tab_s1):
                                ...
                                ).classes('w-full').style('min-height: 250px; font-size: 11px; font-family: monospace;')
