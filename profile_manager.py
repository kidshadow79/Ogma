...
class ProfileManager:
    ...
    def _create_minimal_defaults(self) -> Dict:
        ...
        return {
            ...
            "ego_prompt_default": """# EGO PROMPT - SYSTÈME DE RÉFÉRENCES MÉMORIELLES
...
"""
    ...
    def _calculate_data_sizes(self) -> Dict:
        ...
        data_folders = [
            'conversations', 'generated_images', 'summaries_cache', 
    ...
    def save_current_profile(self, profile_name: str, description: str = "") -> Tuple[bool, str, Optional[Path]]:
        ...
        try:
            ...
            # 5. Créer un rapport de sauvegarde
            backup_report = {
    ...
    def delete_current_profile(self, confirmation_code: str, preserve_founders: bool = True) -> Tuple[bool, str]:
        ...
        try:
            ...
            folders_to_delete = [
                'conversations', 'generated_images', 'summaries_cache',
                'uploads', 'biographies', 'ego_archive'
            ]
            ...
            self._reset_ego_to_default()
            deleted_items.append("🎭 Ego réinitialisé")
            print("  ✅ Ego remis par défaut")
    ...
    def _reset_settings_to_defaults(self):
        """Remet les instructions dans settings.json aux valeurs par défaut"""
        ...
        try:
            ...
            print(f"⚠️ Erreur reset settings : {e}")
    ...
    def _reset_identities_to_defaults(self):
        ...
        try:
            default_identities = self.defaults.get('identities_defaults', {})
    ...
    def _reset_ego_to_default(self):
        """Recrée ego_prompt.txt avec la structure générique par défaut"""
        
        ego_file = self.data_root / "ego_prompt.txt"
        
        try:
            default_ego = self.defaults.get('ego_prompt_default', '')
            
            if default_ego:
                with open(ego_file, 'w', encoding='utf-8') as f:
                    f.write(default_ego)
                print(f"  ✅ ego_prompt.txt recréé avec structure par défaut")
            else:
                print(f"  ⚠️ Pas de template ego par défaut trouvé")
                
        except Exception as e:
            print(f"⚠️ Erreur reset ego : {e}")
    ...
    def load_profile_backup(self, backup_path: Path) -> Tuple[bool, str]:
        ...
        try:
            ...
            # 4. Vérifier l'intégrité de la restauration
...
Le profil est maintenant actif."""
            ...
            # 5. Réinitialiser le MemoryManager pour prendre en compte les nouvelles données
