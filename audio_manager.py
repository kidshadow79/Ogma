...
def clean_text_for_tts(text: str) -> str:
    """
    Nettoie le texte pour la synthèse vocale en supprimant les émojis et caractères problématiques.
    
    Args:
        text: Texte à nettoyer
        
    Returns:
        Texte nettoyé sans émojis
    """
    if not text:
        return text
    
    # Pattern pour détecter les émojis (plages Unicode principales)
...
class AudioManager:
    ...
    def __init__(self, use_whisper_api: bool = False, api_key: Optional[str] = None):
        ...
        self.tts_engine_type: str = "system"  # "system", "google", "elevenlabs", "azure", "gtts", "edge_tts"
    ...
    async def record_once(self, timeout: float = 8.0) -> Optional[str]:
        """
        Enregistre un audio et le transcrit (mode push-to-talk).
        Version optimisée pour éviter les déconnexions.
        
        Args:
            timeout: Durée max d'enregistrement en secondes (réduit pour stabilité)
        ...
        try:
            ...
            print(f"[AUDIO] 🔴 Enregistrement ({actual_timeout}s max)...")
            ...
            with self.microphone as source:
                ...
                # Enregistrement avec timeout court et phrase_time_limit court
                audio_data = self.recognizer.listen(
                    source, 
                    timeout=actual_timeout, 
                    phrase_time_limit=actual_timeout
                )
    ...
    def configure_tts_engine(self, engine_type: str, **kwargs):
        ...
        if engine_type == "google":
            ...
            print(f"[TTS] Moteur ElevenLabs configuré: voice_id={self.elevenlabs_voice_id}")
